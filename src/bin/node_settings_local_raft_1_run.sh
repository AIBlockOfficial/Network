...
if [ "$USE_CA_NODES" = "1" ]
then
    RUST_LOG="$MINER_LOG" target/release/node miner --config=src/bin/node_settings_local_raft_1.toml --address "127.0.0.1:12520" --with_user_address="127.0.0.1:12521" --tls_certificate_override="$(cat <src/bin/tls_data/miner101.bundle.pem)" --tls_private_key_override="$(cat <src/bin/tls_data/miner101.key)" --api_port=3010> miner_0.log 2>&1 &
    m0=$!
    ...
    RUST_LOG="$MINER_LOG" target/release/node miner --config=src/bin/node_settings_local_raft_1.toml --with_user_index=1 --api_port=3010> miner_0.log 2>&1 &
    m0=$!

...
use system::upgrade::{dump_db, get_db_to_dump_no_checks, DbSpecInfo, DB_SPEC_INFOS};
...
async fn main() {
    ...
    if Processing::Read == processing {
        process_read(db_modes);
    } else {
        println!("Start Upgrade with config {:?}, {:?}", processing, db_modes);
        unimplemented!();
    }
}

/// Process reading databases, format in a rust ready constants.
fn process_read(db_modes: Vec<(String, DbMode)>) {
    ...
    for (node_type, mode) in db_modes {
        for spec in DB_SPEC_INFOS.iter().filter(|s| s.node_type == node_type) {
            ...
            let db = get_db_to_dump_no_checks(mode, spec, None).unwrap();

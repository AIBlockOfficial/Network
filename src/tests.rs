...
async fn create_third_block_common(network_config: NetworkConfig, cfg_num: CfgNum) {
    ...
    let mut network = Network::create_from_config(&network_config).await;
    let compute_nodes = &network_config.compute_nodes;
    let c_maj_mined = &node_select(compute_nodes, CfgNum::Majority);

    let (transactions, t_hash, tx) = valid_transactions(true);
    let (_, block_info0) = complete_block(0, Some("0"), &BTreeMap::new(), c_maj_mined.len());
    let (_, block_info1) = complete_block(1, Some("0"), &BTreeMap::new(), c_maj_mined.len());
    let block0_mining_txs = complete_block_mining_txs(&block_info0);
    let block1_mining_txs = complete_block_mining_txs(&block_info1);

    create_first_block_act(&mut network).await;
    compute_all_mining_block_mined(&mut network, c_maj_mined, &block_info0).await;
    send_block_to_storage_act(&mut network, CfgNum::Majority).await;

    create_block_act(&mut network, Cfg::All, cfg_num).await;
    compute_all_mining_block_mined(&mut network, c_maj_mined, &block_info1).await;
    send_block_to_storage_act(&mut network, CfgNum::Majority).await;
    ...
    assert_eq!(
        ...
        node_all_or(compute_nodes, CfgNum::Majority, None, Some(vec![]))
    );
    assert_eq!(
        block_transaction_after,
        node_all(compute_nodes, Some(vec![t_hash]))
    ...
    let expected_utxo = merge_txs_3(&block0_mining_txs, &block1_mining_txs, &transactions);
    assert_eq!(len_and_map(&utxo_set_after[0]), len_and_map(&expected_utxo));
    assert_eq!(equal_first(&utxo_set_after), node_all(compute_nodes, true));
